...
const createIngredient = catchAsync(async (req, res, next) => {
...
const getIngredientsByRecipeId = catchAsync(async (req, res, next) => {
...
const markAsShopped = catchAsync(async (req, res, next) => {
  const { ingredientId } = req.params;
  const ingredient = await Ingredient.findByIdAndUpdate(ingredientId, {
    isShopped: true,
  });
  res.status(200).json({
    status: 'success',
